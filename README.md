...
## If the app says "Cloud unavailable"

This status now includes the API error detail. Common causes:

1. `DATABASE_URL` is missing in Vercel Project Settings → Environment Variables.
2. You opened a non-Vercel URL (for example a local static server), so `/api/stones` is not available.
3. Neon connection string is invalid or database access is blocked.

Quick check:
- Open `https://YOUR-VERCEL-APP.vercel.app/api/stones` in a browser.
- If it returns JSON with `records`, cloud is working.
- If it returns JSON with `error`, fix that message in Vercel/Neon settings and redeploy.
